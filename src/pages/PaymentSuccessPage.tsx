  ...
  return (
    <div className="min-h-screen bg-dough-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-warm-lg w-full max-w-sm p-8 text-center space-y-5">
        ...
        <div>
          <h1 className="font-display text-2xl font-bold text-oven-800">Pembayaran Berhasil!</h1>
          {store && (
            <p className="font-body text-sm text-crust-500 mt-1">
              Paket <strong>{PLAN_LABELS[plan]}</strong> untuk <strong>{store}</strong>
            </p>
          )}
        </div>

        <div className="bg-dough-50 border border-dough-200 rounded-xl px-4 py-4 text-left space-y-2">
          <p className="font-body text-sm font-semibold text-oven-800">Langkah selanjutnya:</p>
          <ol className="space-y-1.5">
            <li className="font-body text-sm text-crust-600 flex items-start gap-2">
              <span className="font-semibold text-crust-500 flex-shrink-0">1.</span>
              Tim kami akan menghubungi {email || 'email Anda'} dalam 1×24 jam untuk aktivasi akun
            </li>
            <li className="font-body text-sm text-crust-600 flex items-start gap-2">
              <span className="font-semibold text-crust-500 flex-shrink-0">2.</span>
