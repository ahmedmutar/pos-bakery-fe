...
const STATUS_FLOW: { key: OrderStatus; label: string; color: string; bg: string }[] = [
  ...
  { key: 'IN_PRODUCTION', label: 'Diproduksi',   color: 'text-amber-700',  bg: 'bg-amber-100' },
  { key: 'READY',         label: 'Siap Ambil',   color: 'text-green-700',  bg: 'bg-green-100' },
  { key: 'COMPLETED',     label: 'Selesai',      color: 'text-crust-600',  bg: 'bg-crust-100' },
]
...
const NEXT_ACTION: Partial<Record<OrderStatus, { label: string; color: string }>> = {
  ...
  READY:         { label: 'Selesaikan Pesanan', color: 'bg-crust-600 hover:bg-crust-700 text-cream' },
}
      ...
      <div className="bg-white rounded-2xl shadow-warm-lg w-full max-w-lg max-h-[90vh] flex flex-col">
        ...
        <div className="overflow-y-auto flex-1 px-6 py-5 space-y-5 scrollbar-thin">
                  ...
                  return (
                    <div key={s.key} className="flex items-center flex-1">
                      <div className={cn(
                        ...
                        isCurrent ? 'bg-crust-600 ring-4 ring-crust-100' :
                        isPast ? 'bg-crust-500' : 'bg-dough-200'
                      )}>
                      ...
                      {i < STATUS_FLOW.length - 1 && (
                        <div className={cn(
                          ...
                          i < currentIdx ? 'bg-crust-500' : 'bg-dough-200'
                        )} />
                      )}
                    </div>
                  )
              ...
              <div className="flex justify-between">
                {STATUS_FLOW.map((s, i) => (
                  <p key={s.key} className={cn(
                    ...
                    i === currentIdx ? 'text-crust-700 font-semibold' : 'text-crust-400'
              ...
              {order.items.map((item) => (
                <div key={item.id} className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    ...
                    {item.customNotes && (
                      <p className="font-body text-xs text-crust-400 italic mt-0.5">"{item.customNotes}"</p>
                    )}
                  </div>
          ...
          {order.remainingAmount > 0 && !isCancelled && (
            <div>
              {showPay ? (
                <div className="space-y-3 bg-dough-50 rounded-xl p-4">
                  <p className="font-body text-sm font-medium text-oven-800">Catat Pelunasan</p>
